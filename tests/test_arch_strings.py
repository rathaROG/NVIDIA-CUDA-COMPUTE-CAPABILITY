...
from nvidia_arch.core import normalize_arch_list, validate_arch_string, make_gencode_flags
...
def test_normalize_arch_list(input_str, expected):
    assert normalize_arch_list(input_str) == expected
...
def test_make_gencode_flags_accepts_all(input_str):
    ...
    assert any(",code=[" in f for f in flags)  # PTX
    assert len(flags) >= 2
...
def test_normalize_empty_and_whitespace(input_str):
    assert normalize_arch_list(input_str) == ""
